...
def parse(source):
    ...
    def t_NAME(t):                      # 1文字目は英字 or _ 2文字目以降は英字 or 数字 or _
        r'[a-zA-Z_][a-zA-Z0-9_]*'       # 1文字目 [a-zA-Z_] は 英字（a〜z または A〜Z）または _                                        # 2文字目 [a-zA-Z0-9_]* の * は0以上の繰返し(1文字目のみも可能)
    ...
    # FLOAT を NUMBERより、先に記述する！！！
    def t_FLOAT(t):                     # 実数
        r'\d+\.\d+'                     # 1桁以上の整数 . 1桁以上の整数
        t.value = float(t.value)        # 文字列 から 実数に
        return t
    ...
    def t_STRING(t):
        r'"[^"]*"'                      # ""で囲まれた文字の繰り返し　(同じ行の "a" "b" などに対応するため、中に " があるものを除く)   
        t.value = t.value[1:-1]         # ""を除いた文字
        return t
    ...
    def p_init_array_num_multi(p):  # 1, 2, 3 配列に入れる初期値が複数
        ...
        'init_array_val : init_array_val COMMA value'  # 1,2       ,     3
        # 配列のリストに加える(数値を配列にしてから)
        p[0] = p[1] + [p[3]]

    def p_init_array_num_single(p): 
        'init_array_val : value'
        # 配列のリストに
        p[0] = [p[1]]



    def p_init_array_undif(p):      # 配列の初期値にn個の未定義の値
        'init_array : L_C_BRACKET value KO NO UNDEF NO VALUE R_C_BRACKET'

        n = p[2]

        # 値のチェック
        if isinstance(n, int):
            # "未定義"をn回、繰り返したリストを作って渡す
            p[0] = ["未定義"] * n 
        
        else:                       # nが整数値でない
            message = f"エラー\n{currentLine + 1}行目:   参照した値が整数ではありません"
            print(message, file=sys.stderr)
            p.parser.error = message
    ...
    # 配列の宣言と同時に関数の戻り値を代入  整数型の配列: array ← func()
    def p_declaration_func_call_array(p):
        'statement :  TYPE NO ARRAY COLON NAME ASSIGN NAME L_PAREN args R_PAREN'
    ...
    def p_pow_function_call(p):
        'value : POW L_PAREN args R_PAREN'
    ...
    def p_conditional_divisible_multi(p):   # x が y と z で割り切れる
        'conditional : value GA value TO value DE DIVISIBLE'
        ...
        # 値が未定義ならreturn (エラーメッセージはvalueに取得したときに作成済み)
        if (valuea is None) or (valueb is None) or (valuec is None):
            return
        
        # 0 で割ろうとしたらエラー
        if (valueb == 0) or (valuec == 0):
            message = f"エラー\n{currentLine + 1}行目:   0 で除算することはできません"
            print(message, file=sys.stderr)
            p.parser.error = message
            return
        ...
        # 割り切れるか判定して、結果を入れる
        if (valuea % valueb == 0) and (valuea % valuec == 0):
            p[0] = True
        else:
            p[0] = False
    ...
    # 小数点以下を切り上げた値
    def p_rounded_up(p):
        'value : value NO DECIMAL_P BELOW WO ROUNDED_UP VALUE'
    ...
    # ブレークポイントに到達したことを知らせる（メッセージをなくしたので必要なし）
    # if isbreak == True:
    #     result["isbreak"] = True
